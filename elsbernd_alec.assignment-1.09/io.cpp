...
void io_handle_input(dungeon *d)
{
  ...
  do {
	...
	for(int i = 0; i < 10; i++){
	  if(d->inventory[i] == NULL){
	    d->inventory[i] = d->objmap[d->PC->position[dim_y]][d->PC->position[dim_x]];
    ...
    case 'i':
      clear();
      charArray = "";
      if(d->inventory[0] != NULL){
	charArray = d->inventory[0]->name.c_str();
      }
      mvprintw(6, 10, "(1)  %s",charArray);
      charArray = "";
      if(d->inventory[1] != NULL){
	charArray = d->inventory[1]->name.c_str();
      }
      mvprintw(7, 10, "(2)  %s",charArray);
      charArray = "";
      if(d->inventory[2] != NULL){
	charArray = d->inventory[2]->name.c_str();
      }
      mvprintw(8, 10, "(3)  %s",charArray);
      charArray = "";
      if(d->inventory[3] != NULL){
	charArray = d->inventory[3]->name.c_str();
      }
      mvprintw(9, 10, "(4)  %s",charArray);
      charArray = "";
      if(d->inventory[4] != NULL){
	charArray = d->inventory[4]->name.c_str();
      }
      mvprintw(10, 10, "(5)  %s",charArray);
      charArray = "";
      if(d->inventory[5] != NULL){
	charArray = d->inventory[5]->name.c_str();
      }
      mvprintw(11, 10, "(6)  %s",charArray);
      if(d->inventory[6] != NULL){
	charArray = d->inventory[6]->name.c_str();
      }
      mvprintw(12, 10, "(7)  %s",charArray);
      charArray = "";
      if(d->inventory[7] != NULL){
	charArray = d->inventory[7]->name.c_str();
      }
      mvprintw(13, 10, "(8)  %s",charArray);
      charArray = "";
      if(d->inventory[8] != NULL){
	charArray = d->inventory[8]->name.c_str();
      }
      mvprintw(14, 10, "(9)  %s",charArray);
      charArray = "";
      if(d->inventory[9] != NULL){
	charArray = d->inventory[9]->name.c_str();
      }
      mvprintw(15, 10, "(10)  %s",charArray);
      while (getch() == 0)
      ;
      clear();
      fail_code = 1;
      ...
      clear();
      charArray = "";
      if(d->equipment[0] != NULL){
	charArray = d->equipment[0]->name.c_str();
      }
      mvprintw(6, 10, "WEAPON: %s",charArray);
      charArray = "";
      if(d->equipment[1] != NULL){
	charArray = d->equipment[1]->name.c_str();
      }
      mvprintw(7, 10, "OFFHAND: %s",charArray);
      charArray = "";
      if(d->equipment[2] != NULL){
	charArray = d->equipment[2]->name.c_str();
      }
      mvprintw(8, 10, "RANGED: %s",charArray);
      charArray = "";
      if(d->equipment[3] != NULL){
	charArray = d->equipment[3]->name.c_str();
      }
      mvprintw(9, 10, "LIGHT: %s",charArray);
      charArray = "";
      if(d->equipment[4] != NULL){
	charArray = d->equipment[4]->name.c_str();
      }
      mvprintw(10, 10, "ARMOR: %s",charArray);
      charArray = "";
      if(d->equipment[5] != NULL){
	charArray = d->equipment[5]->name.c_str();
      }
      mvprintw(11, 10, "HELMET: %s",charArray);
      if(d->equipment[6] != NULL){
	charArray = d->equipment[6]->name.c_str();
      }
      mvprintw(12, 10, "CLOAK: %s",charArray);
      charArray = "";
      if(d->equipment[7] != NULL){
	charArray = d->equipment[7]->name.c_str();
      }
      mvprintw(13, 10, "GLOVES: %s",charArray);
      charArray = "";
      if(d->equipment[8] != NULL){
	charArray = d->equipment[8]->name.c_str();
      }
      mvprintw(14, 10, "BOOTS: %s",charArray);
      charArray = "";
      if(d->equipment[9] != NULL){
	charArray = d->equipment[9]->name.c_str();
      }
      mvprintw(15, 10, "AMULET:  %s",charArray);
      charArray = "";
      if(d->equipment[10] != NULL){
	charArray = d->equipment[10]->name.c_str();
      }
      mvprintw(16, 10, "RING 1:  %s",charArray);
      charArray = "";
      if(d->equipment[11] != NULL){
	charArray = d->equipment[11]->name.c_str();
      }
      mvprintw(17, 10, "RING 2:  %s",charArray);
      while (getch() == 0)
      ;
      fail_code = 1;
      clear();
	...
	cursorPos = 0;
	do {
	   if(getchar == KEY_UP && cursorPos > 0){
	cursorPos--;
      }
      if(getchar == KEY_DOWN && cursorPos < 9){
	cursorPos++;
      }
	  clear();
      charArray = "";
	...
	charArray = d->inventory[9]->name.c_str();
      ...
      mvprintw(15, 10, "(10)  %s",charArray);
      mvprintw(6 + cursorPos, 8, "*");
      mvprintw(17, 10, "Scroll with up and down arrows --- Confirm with w");
      mvprintw(4, 10, "Wear which item?");
      refresh();
       getchar = getch();
	} while (getchar != 119);
	if(d->inventory[cursorPos] != NULL){
	  if(d->inventory[cursorPos]->type > 0 && d->inventory[cursorPos]->type < 11){
	    ...
	    if(d->equipment[d->inventory[cursorPos]->type -1] != NULL){
	      ...
	      d->inventory[cursorPos] = temp;
	    }
	    else{
	      d->equipment[d->inventory[cursorPos]->type-1] = d->inventory[cursorPos];
	       d->inventory[cursorPos] = NULL;
	    }
	  }
	  else if(d->inventory[cursorPos]->type == 11){
	    if(d->equipment[d->inventory[cursorPos]->type-1] == NULL){
	       d->equipment[d->inventory[cursorPos]->type-1] = d->inventory[cursorPos];
	       d->inventory[cursorPos] = NULL;
	    }
	    else{
	      if(d->equipment[d->inventory[cursorPos]->type] == NULL){
		 d->equipment[d->inventory[cursorPos]->type] = d->inventory[cursorPos];
	       d->inventory[cursorPos] = NULL;
	      }
	      else{
	      temp = d->equipment[d->inventory[cursorPos]->type-1];
	...
	charArray = d->inventory[9]->name.c_str();
      ...
      mvprintw(15, 10, "(10)  %s",charArray);
      mvprintw(6 + cursorPos, 8, "*");
      mvprintw(17, 10, "Scroll with up and down arrows --- Confirm with d");
      mvprintw(4, 10, "Drop which item?");
      refresh();
       getchar = getch();
	} while (getchar != 100);
       if(d->inventory[cursorPos] != NULL &&  d->objmap[d->PC->position[dim_y]][d->PC->position[dim_x]] == NULL){
	 d->objmap[d->PC->position[dim_y]][d->PC->position[dim_x]] = d->inventory[cursorPos];
	 d->inventory[cursorPos] = NULL; 
	}
	fail_code = 1;
	clear();
	...
	charArray = d->inventory[9]->name.c_str();
      ...
      mvprintw(15, 10, "(10)  %s",charArray);
      mvprintw(6 + cursorPos, 8, "*");
      mvprintw(17, 10, "Scroll with up and down arrows --- Confirm with x");
      mvprintw(4, 10, "Expunge which item?");
      refresh();
       getchar = getch();
       } while (getchar != 120);
	 d->inventory[cursorPos] = NULL; 
	...
	charArray = d->inventory[9]->name.c_str();
      ...
      mvprintw(15, 10, "(10)  %s",charArray);
      mvprintw(6 + cursorPos, 8, "*");
      mvprintw(17, 10, "Scroll with up and down arrows --- Confirm with I");
      mvprintw(4, 10, "Description for which item?");
      refresh();
       getchar = getch();
       } while (getchar != 73);
       ...
       if(d->inventory[cursorPos] != NULL){
	 clear();
	 charArray = d->inventory[cursorPos]->description.c_str();
	 mvprintw(6, 0, "Item Description: \n%s",charArray);
	 while (getch() == 0);
       }
       fail_code = 1;
       clear();
	 ...
	 if(getchar == KEY_DOWN && cursorPos < 11){
	   cursorPos++;
	 }
	  clear();
	...
	charArray = d->equipment[11]->name.c_str();
      ...
      mvprintw(6 + cursorPos, 8, "*");
      mvprintw(19, 10, "Scroll with up and down arrows --- Confirm with t");
      mvprintw(4, 10, "Takeoff which item?");
      refresh();
       getchar = getch();
       } while (getchar != 116);
  ...
  * not otherwise used.                                                 */
  ...
  } while (getchar != 't' && getchar != 27);

  if(getchar ==  't'){
    if(d->character_map[dest[dim_y]][dest[dim_x]] != NULL){
	clear();
	charArray = d->character_map[dest[dim_y]][dest[dim_x]]->name;
	mvprintw(6, 10, "Monster Name: %s",charArray);
	mvprintw(7, 10, "Monster HP: %d", (int) d->character_map[dest[dim_y]][dest[dim_x]]->hp);
	mvprintw(8, 10, "Monster Speed: %d",(int) d->character_map[dest[dim_y]][dest[dim_x]]->speed );
